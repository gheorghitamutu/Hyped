...
namespace Viridian.Msvm.Threshold
{
    public class VirtualSystemSnapshotSettingData : MsvmBase
    {
         ...
         */
        public bool IgnoreNonSnapshottableDisks
        {
            get
            {
                if (LateBoundObject[nameof(IgnoreNonSnapshottableDisks)] == null)
                {
                    return System.Convert.ToBoolean(0);
                }
                return (bool)LateBoundObject[nameof(IgnoreNonSnapshottableDisks)];
            }
            set
            {
                LateBoundObject[nameof(IgnoreNonSnapshottableDisks)] = value;
                if ((IsEmbedded == false) && (AutoCommit == true))
                {
                    PrivateLateBoundObject.Put();
                }
